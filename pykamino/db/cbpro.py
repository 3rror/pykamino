...
def msg_to_order(msg) -> (Order, OrderTimeline):
    """
    Convert a Coinbase message into an `OrderTimeline` instance, and the related `Order` instance if any.

    Returns:
        order: an Order instance if available, else None
        timeline: an OrderTimeline instance
    Raises:
    ...
    try: reason = msg['reason']
    # reason can be null
    except KeyError: reason = None
